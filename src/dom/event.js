...
const event = (gameState) => {
      ...
      if(playerGameboard) {
        for (let i = 0; i < shipLength; i += 1) {
          ...
          if (playerGameboard.getOrientation()) {
            hoverCell = board.querySelector(`.cell[data-cell="${rowIndex}, ${colIndex + i}"]`);
          } else {
            hoverCell = board.querySelector(`.cell[data-cell="${rowIndex + i}, ${colIndex}"]`);
          }
  ...
  const nextHandler = (target) => {
    if(gameState.getShipFactory(gameState.player1).allShipsPlaced()){
      ...
      const playerBoard = document.querySelector('.ship-placement-player-1 .board-wrapper')
  ...
  const isGameStarted = () => {
    let playerBoard;
    if(gameState.getShipFactory(gameState.player1).allShipsPlaced() && gameState.getShipFactory(gameState.player2).allShipsPlaced()){
      gameState.startGame()
      gameStartHandler()
    }else if (!gameState.player2.isHuman) {
      playerBoard = document.querySelector('.ship-placement-player-1 .board-wrapper');
      shakeHandler(playerBoard)
    } else {
      playerBoard = document.querySelector('.ship-placement-player-2 .board-wrapper');
      shakeHandler(playerBoard)
    }
  }
  ...
  const handleShipPlacementEvents = (target) => {
    if(target.closest('.ship-placement-container.p2 .next-btn') || target.closest('.ship-placement-container.one-player .next-btn')) {
      isGameStarted()
  ...
  const handleBotAttack = (player1Section, player2Section) => {
    setTimeout(() => {
      ...
      const cell = document.querySelector(`.player-1-section .cell[data-cell="${aix}, ${aiy}"]`)
