const ComputerPlayer = () => {
  ...
  const isCoordinateAlreadyAttacked = (x, y) =>
  ...
  const setAttackCoordinate = () => {
    if (latestCoordinate.length > 0) {
      ...
      while (nextDirectionIndex < directionVectors.length) {
        ...
        const candidateX =
          latestCoordinate[latestCoordinate.length - 1][0] + dx;
        const candidateY =
          latestCoordinate[latestCoordinate.length - 1][1] + dy;
        ...
        if (
          ...
          candidateY >= 0 &&
          candidateY < 10 &&
          !isCoordinateAlreadyAttacked(candidateX, candidateY)
        ) {
